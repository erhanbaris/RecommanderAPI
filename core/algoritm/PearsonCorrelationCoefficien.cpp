...
vector<pair<PRODUCT_TYPE, float>> core::algoritm::PearsonCorrelationCoefficien::Recommend(const CUSTOM_MAP<PRODUCT_TYPE, vector<Rating>> *users, const USER_TYPE &userId) {
    ...
    for (auto it = user->second.cbegin(); it != userEnd; ++it) {
        ...
        for (auto it2 = indexResult2.begin(); it2 != indexResult2End; ++it2) {
            ...
            if (indexItem->UserId == userId || userInfo.TotalProduct <= core::AvgProductCount ||
                userInfo.AvgRating <= core::AvgRating)
                continue;
...
vector<pair<PRODUCT_TYPE, float>> core::algoritm::PearsonCorrelationCoefficien::CompareUser(CUSTOM_MAP<PRODUCT_TYPE, RATE_TYPE> &user1,
                                              vector<pair<PRODUCT_TYPE, IndexItem *>> &comparableUsers) const {
    float sum_xy = 0;
    float sum_x = 0;
    float sum_y = 0;
    float sum_x2 = 0;
...
void core::algoritm::PearsonCorrelationCoefficien::SetProductIndex(CUSTOM_MAP<PRODUCT_TYPE, vector<IndexItem *>> *pProductIndex) {
    this->productIndex = pProductIndex;
}

void core::algoritm::PearsonCorrelationCoefficien::SetUserIndex(CUSTOM_MAP<USER_TYPE, UserInfo> *pUserIndex) {
